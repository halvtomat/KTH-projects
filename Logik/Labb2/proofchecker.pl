...
check_proof(Prems, Goal, Proof):-
    retractall(prems_db(_)),                        %Make sure the premiss database is clear
    retractall(proof_db(_)),                        %Make sure the proof database is clear
    ...
    last(Proof,[_,Goal,_]),                         %Check if the proof actually leads to the goal
    check_rule(Proof).                              %Check if the proof follows the rules
...
check_rule([[[N,X,assumption]|T1]|T2]):-                                                                                 %|
    ...
    asserta(proof_db(Prootemp)),                    %Add the new list to the proof database                              %|
    check_rule(T1),                                                                                                      %|
    %--------                                                                                                            %|
    last([[N,X,assumption]|T1], Res),               %Get the last line in the box                                        %|
    append(Proo,[[[N,X,assumption],Res]],Proo2),    %Combine the original proof with the first and last line of the box  %|
    retractall(proof_db(_)),                        %Remove everything from the proof database                           %|
    asserta(proof_db(Proo2)),                       %Add the new proof (proof2) to the proof database                    %|
    check_rule(T2).                                 %Continue checking the rest of the proof                             %|
%–––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––:
